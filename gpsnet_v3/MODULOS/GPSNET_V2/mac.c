/**********************************************************************************
@file   mac.c
@brief  GPSNET MAC-layer functions
...
em conjunto com o gpsnet packet */
volatile MAC_FRAME_CONTROL mac_frame_control;
volatile GPSNET_PACKET     gpsnet_packet; 

/* O vetor gpsnet_beacon guarda uma quantidade "BeaconLimit" 
de beacons durante o active scan de um canal  */
volatile GPSNET_BEACON     gpsnet_beacon[BeaconLimit];
...
// Analiza se o beacon é valido na rede GPSNET
...
void MAC_Beacon(void){
  ...
  PHYSetLongRAMAddr(15, BetterDepth);
  PayloadSize++;
...
INT8U GPSNET_Associate(void)
{
  ...
  AssociateLoop:
   ...
   {
      if (gpsnet_beacon[i].DeviceDepth < aux)
      {
        // Verifica se já não houve tentativa de associação negada
        if(gpsnet_beacon[i].AssociationStatus == 0)
        {
            aux = gpsnet_beacon[i].DeviceDepth;
            j = i;
        }
      }
   }
   ...
   if (aux == 255)
   {
      ...
      for(i=0;i<BeaconLimit;i++)
      {
          ...
          gpsnet_beacon[i].DeviceDepth = 255;
          BeaconCnt = 0;
      }      
      return FALSE;
   }
  ...
  while(z<3) 
  {
   ...
   macACK = FALSE;
   MAC_Command(ASSOCIATION_REQUEST,MAC_ACK_INTRA_PAN,gpsnet_beacon[j].PAN_Ident,gpsnet_beacon[j].Addr_16b);
   ...
   if (macACK == TRUE)
   {
      ...
      PHYSetShortRAMAddr(WRITE_PANIDL,(INT8U)(gpsnet_beacon[j].PAN_Ident&0xFF));
      PHYSetShortRAMAddr(WRITE_PANIDH,(INT8U)(gpsnet_beacon[j].PAN_Ident>>8));
      
      macPANId = gpsnet_beacon[j].PAN_Ident;
      
      // Data Request
      macACK = FALSE;
      MAC_Command(DATA_REQUEST,MAC_ACK_INTRA_PAN,gpsnet_beacon[j].PAN_Ident,gpsnet_beacon[j].Addr_16b);
      ...
      if (macACK == TRUE)
      {
         ...
         if (mac_packet.MAC_Payload[0] == 0x02)
         {
            ...
            // Se diferente de 0, alocar motivo em gps_beacon[j].AssociationStatus      
            ...
            }else
            {
                ...
                gpsnet_beacon[j].AssociationStatus = mac_packet.MAC_Payload[3];
  ...
  gpsnet_beacon[j].DeviceDepth = 0xFF;
  goto AssociateLoop;
