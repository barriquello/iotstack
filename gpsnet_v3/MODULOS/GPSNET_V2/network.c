...
INT8U UpRoute(INT8U RouteInit, INT8U NWKPayloadSize)
{
    ...
    {        
        ...
        my_distance = (INT32U) ((dest_lat.x - gps_latitude.x) * (dest_lat.x - gps_latitude.x)) + ((dest_long.y - gps_longitude.y) * (dest_long.y - gps_longitude.y));        
        ...
        {          
          ...
          if ((nwk_packet.NWK_Parameter&NWK_GPSR) == NWK_GPSR) 
          {
            ...
            nearest_distance = (INT32U)((dest_lat.x - better_lat.x) * (dest_lat.x - better_lat.x)) + ((dest_long.y - better_long.y) * (dest_long.y - better_long.y));
          ...
          {                      
            ...
            if (gpsnet_neighbourhood[i].NeighborGPS_Latitude.x != INSTALLER_LAT) 
            {
              ...
              {                          
                ...
                neighbor_distance = (INT32U)((dest_lat.x - gpsnet_neighbourhood[i].NeighborGPS_Latitude.x) * (dest_lat.x - gpsnet_neighbourhood[i].NeighborGPS_Latitude.x)) + ((dest_long.y - gpsnet_neighbourhood[i].NeighborGPS_Longitude.y) * (dest_long.y - gpsnet_neighbourhood[i].NeighborGPS_Longitude.y));
        ...
        ContinueGPSRRouting:  
          ...
          for(i=0;i<NEIGHBOURHOOD_SIZE;i++)
          {
            ...
            {          
              ...
              {        				
        				// planarização
        				my_distance = (INT32U)((gpsnet_neighbourhood[i].NeighborGPS_Latitude.x - gps_latitude.x) * (gpsnet_neighbourhood[i].NeighborGPS_Latitude.x - gps_latitude.x)) + ((gpsnet_neighbourhood[i].NeighborGPS_Longitude.y - gps_longitude.y) * (gpsnet_neighbourhood[i].NeighborGPS_Longitude.y - gps_longitude.y));
...
/*        				
        					...
        					{
*/        					
                  my_distance = (INT32U)((gps_latitude.x - gpsnet_neighbourhood[j].NeighborGPS_Latitude.x) * (gps_latitude.x - gpsnet_neighbourhood[j].NeighborGPS_Latitude.x)) + ((gps_longitude.y - gpsnet_neighbourhood[j].NeighborGPS_Longitude.y) * (gps_longitude.y - gpsnet_neighbourhood[j].NeighborGPS_Longitude.y));
                  //my_distance = SquareRoot(my_distance);
                  //dcj = my_distance * my_distance;
                  dcj = my_distance;
                  
                  my_distance = (INT32U)((gpsnet_neighbourhood[i].NeighborGPS_Latitude.x - gpsnet_neighbourhood[j].NeighborGPS_Latitude.x) * (gpsnet_neighbourhood[i].NeighborGPS_Latitude.x - gpsnet_neighbourhood[j].NeighborGPS_Latitude.x)) + ((gpsnet_neighbourhood[i].NeighborGPS_Longitude.y - gpsnet_neighbourhood[j].NeighborGPS_Longitude.y) * (gpsnet_neighbourhood[i].NeighborGPS_Longitude.y - gpsnet_neighbourhood[j].NeighborGPS_Longitude.y));
                  //my_distance = SquareRoot(my_distance);
