...
#if (defined ISR_STACK_ENABLE && (ISR_STACK_ENABLE == 1))
  OS_CPU_TYPE  ISR_STACK[ISR_STACK_SIZE] = {0x49535253, 0x5441434B};
  INT32U       SPvalue_bkp = (INT32U)&ISR_STACK[ISR_STACK_SIZE-2];
#endif
...
void TickTimerSetup(void)
{
  ...
  /* ### Init_TPM init code */
  TIMER_CTRL = 0x00;                       /* Stop and reset counter */
  TIMER_MODULE = (configCPU_CLOCK_HZ / configTICK_RATE_HZ) >> configTIMER_PRE_SCALER; /* Period value setting */
  (void)(TIMER_CTRL == 0);                 /* Overflow int. flag clearing (first part) */
  /* TPMxSC: TOF=0,TOIE=1,CPWMS=0,CLKSB=0,CLKSA=1,PS2=0,PS1=0,PS0=0 */
  TIMER_CTRL = 0x48 | configTIMER_PRE_SCALER;                       /* Int. flag clearing (2nd part) and timer control register setting */
...
#if (defined STACK_CHECK) && (STACK_CHECK == 1)  
