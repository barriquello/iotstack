...
INT32U Absolute(INT32S x) {
  ...
  if (x<0) return (INT32U)(-x);
  else return (INT32U)x;
}
...
{                                                       //
  ...
  for(j=0;j<MAX_INT;j++){                               // execute maximum of MAX_INT interactions
    ...
    if((Absolute((INT32S)(Acc - Acc_temp)) < (INT32U)0x0001)||(Acc_temp == 0)) // Test if precision is good enough     
